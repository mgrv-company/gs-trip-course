...
function filtersFor(slot) { return slot === 'meal' ? FOOD_GROUPS.map(g => g.label) : VIBE; }

function renderChips() {
  if (curSlot === 'auto') { $('#optChips').innerHTML = ''; return; }  // '영업중'(섞기) 뷰엔 옵션 칩 없음
  ...
  if (isAuto) {
    ...
    if (curFilter) {
      if (slot === 'meal') {
        ...
        pool = pool.filter(p => (p.v || []).includes(curFilter));
      }
    }
  }
